...
import { fetchMarketData, MarketData } from '@/app/actions/fetch-market-data'; // Import the server action
...
const ASSETS = [
  ...
  { id: "CL", name: "Crude Oil (WTI Futures)", type: "commodity" }, // Changed to CL for WTI Futures
...
// This function now primarily orchestrates calls to the server action and AI flows
async function fetchCombinedDataForAsset(assetId: string, assetName: string, assetType: string, timeframeId: string): Promise<{
  ...
  try {
    // Fetch real market data using the server action
    ...
    if (marketApiData.error && (!marketApiData.price && !marketApiData.rsi && !marketApiData.macd)) {
      // If critical data is missing and there's an error, propagate it
      marketDataError = marketApiData.error;
      console.error(`Critical market data error for ${assetName} (${timeframeId}): ${marketDataError}`);
       // Return early with error and default/empty data for cards
      return {
    ...
    if (marketApiData.error) { // Log non-critical errors but proceed if some data is present
        console.warn(`Partial market data or non-critical error for ${assetName} (${timeframeId}): ${marketApiData.error}`);
        marketDataError = marketApiData.error; // Keep the error to display if needed
    }


    // Use real data if available, otherwise fall back to placeholders or indicate missing data
    ...
    const rsiValue = marketApiData.rsi ?? 50; // Default to neutral if not available
    ...
    // Placeholder for interest rate and news headlines (as these are not from TwelveData basic integration)
    const interestRate = 0.5 + Math.random() * 2; // Simulated interest rate
    ...
    // Specific placeholder news for some assets
    ...
    const tradeRecommendationInput: GenerateTradeRecommendationInput = {
      ...
      sentimentScore: parseFloat(((Math.random() * 2) - 1).toFixed(2)), // Placeholder, ideally from newsSentiment.overallSentiment score
    ...
    const marketOverviewData = { 
      pair: assetName, 
      value: marketApiData.price, // Use real price if available
      change: marketApiData.price ? `${(marketApiData.price * 0.001 * (Math.random() > 0.5 ? 1 : -1)).toFixed(4)} (${(0.1 * (Math.random() > 0.5 ? 1 : -1)).toFixed(2)}%)` : "N/A", // Simulated change
      isPositive: Math.random() > 0.5, // Simulated
    ...
    // Economic indicators remain placeholder
    ...
    return { 
        ...
        marketDataError: marketApiData.error // Pass through any error from market data fetching
    };
...
export default function HomePage() {
  ...
  const [aiData, setAiData] = useState<{
    ...
    technicalIndicatorsData?: any;
    economicIndicatorData?: any;
    marketDataError?: string; // To store errors from market data fetching
  } | null>(null);
